...
library(scales)  # to represent thousdands with commas
...
Here I'm using a different color palette than RalitzaSoultanova. I'm starting from a very nice palette found on [Coolors](https://coolors.co/palette/f4f1de-e07a5f-3d405b-81b29a-f2cc8f) and modfying some colors:
```{r}

col_bg <- "#FAECD6" #"#f2cc8f"  # plot background
...
Get a nice font:
```{r}
sysfonts::font_add(family = "AppleGothic", regular = "AppleGothic.ttf")
```
...
main_df %>% 
  ...
  geom_hline(yintercept = 10000, color = col_elem, linewidth = 0.4) +  # add threshold line
  geom_col() +
  scale_x_date(date_breaks = "1 month", date_labels = "%b") +
  scale_y_continuous(limits = c(0, 35070), expand = c(0, 0)) +
  scale_fill_manual(values = col_steps, labels = c("yes", "no")) +
  labs(title = "My Steps in 2022",
       subtitle = paste("Steps walked:", comma(total_steps), 
                        "\nDaily Average:", comma(avg_steps), 
                        "\nTotal Distance:", comma(total_distance), "km"),
      fill = "10,000 Steps Goal: ") +
  coord_curvedpolar() + 
  theme_void() +
  theme(
    text = element_text(color = col_elem, family = "AppleGothic"),
    plot.margin = margin(0.2, 0.2, 0.2, 0.2, unit = "in"),
    plot.background = element_rect(fill = col_bg, colour = col_bg),
    panel.grid.major.x = element_line(colour = col_elem),
    plot.title = element_text(size = 40, hjust = 0.5, margin = margin(0, 0, 0.1, 0, unit = "in")),
    plot.subtitle = element_text(size = 18, hjust = 0.5, margin = margin(0, 0, 0.1, 0, unit = "in")),
    axis.title = element_blank(),
    axis.text.x = element_text(size = 18, vjust = -0.8),
    legend.title = element_text(size = 18),
    legend.margin = margin(0.1, 0, 0.1, 0, unit = "in"),
    legend.position = "bottom",
    legend.background = element_blank(),
    legend.key.width = unit(0.04, units = "in"),
...
```{r}
ggsave(file = "img/steps.png", width = 15, height = 15, units = "in", dpi = 300)
```