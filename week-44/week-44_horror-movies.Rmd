...
For our second plot, we can define a different color for each sentiment to plot. We're going to use the "nrc" lexicon, which has 10 sentiments, of which we will use 8 (we won't use the general "negative" and "positive"). Even here I start from another beautiful palette by Bluetris, this time the [piano lessons](https://www.color-hex.com/color-palette/58553). Also here, we interpolate the original 5 colors to 8:
```{r}
palette_sentiment <- colorRampPalette(c("#617e7b", "#5c5a52", "#303039", "#564d5f", "#a06e6e"), bias = 1)(8)
...
For this step, we might also want to remove the movies with no overview, and select only variables of interest.  
(We might also want to remove stopwords after tokenization, but this is not necessary since in the end we'll retain only words that are present in the nrc lexicon)
```{r}
token_df <- main_df %>% 
  drop_na(overview) %>% 
  select(id, title, overview, release_year) %>% 
...
Let's count sentiment by year. This is in the end what we want to plot: how the presence of each sentiment in movies' overviews changed across time:
```{r}
...
we might also want to account for the fact that the total number of released movies increased across the years. If we don't, the sentiment count will be influenced by the number of released movies. One way to account for it might be to normalize the count over the number of movies released in each year.  
To do so, let's go back to the "main_df", calculate the number of movies released in each year, and use that to normalize the sentiment count:
```{r}
sentiment_by_year_df <- sentiment_by_year_df %>% 
  left_join(main_df %>%
              drop_na(overview) %>%   # still consider only movies with an overview
              group_by(release_year) %>% 
              summarise(movie_number = n())) %>%  # count number of movies per year
  mutate(sentiment_count_norm = sentiment_count/movie_number)  # normalize sentiment count over movie number
```

## Plotting

It's finally time to plot! We want to plot the number of appearances of each sentiment in the movies' overview across the years. We can go for a line plot, with years on the x-axis and count of sentiments on the y-axis.
...
sentiment_by_year_df %>% ggplot(aes(x = release_year, y = sentiment_count_norm, color = sentiment)) +
